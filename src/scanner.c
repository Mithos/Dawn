 ...
 * 
 * 
 * @return TRUE if tag succesfully imports, FALSE otherwise
 */
static gboolean insert_tag(sqlite3 *db, const gchar *title, const gchar *album, const gchar *artist) {
	
	gchar *zErrMsg = 0; /* SQL error message */
	gint rc; /* SQL return code */
	
	gchar sql[512]; /* create oversized buffer */
	memset(sql, '\0', sizeof(sql));
	
	if( sprintf(sql, "INSERT INTO %s VALUES ('%s', '%s', '%s')", TABLE, title, album, artist) < 1) {
		return FALSE;
	}
...
int main2(int argc, char **argv){
	...
	returnCode = sqlite3_open(argv[1], &db);
	if( returnCode){
